...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "Выберите, что хотите заказать: регистрацию (/registration), гороскоп друидов (/horodru) или тест на депрессию (/beckscale).",
...
def register_handlers_common(dp: Dispatcher, admin_id: int):
    ...
    dp.register_message_handler(secret_command, IDFilter(user_id=admin_id), commands="magic")
